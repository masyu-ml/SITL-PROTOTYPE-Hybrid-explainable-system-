...
class SystemLogicWindow(QDialog):
    def __init__(self):
        ...
        <h2>LAYER 2 & 3: XAI & PHYSICS ARBITRATION</h2>
        <ul>
            <li><b>Layer 2 (ML Engine):</b> Analyzes a 10-sample sliding window. Extracts features: <code>Temp_Diff_Max_mean</code>, <code>Ia_std</code>, and <code>Current_RMS_slope</code> to predict anomalies before Fuzzy thresholds are breached.</li>
...
class NASAMissionControl(QMainWindow):
    ...
    # Thread-safe signal for email updates
    ...
    def __init__(self):
        ...
        self.data_index = 0
        self.paused = True  # Standby
        self.manual_mode = False
        self.continuous_mode = False  # The Bypass Flag
    ...
    def toggle_pause(self):
        if self.df.empty and not self.manual_mode: return
    ...
    def toggle_continuous_mode(self, checked):
        ...
        # If the user clicks this box WHILE the system is currently paused on a critical fault, unpause it!
        if checked and self.paused and not self.df.empty and "CRITICAL" in getattr(self, 'last_ui_state', ''):
            self.paused = False
            self.pause_btn.setText("PAUSE MISSION")
            self.push_notification("E-Stop Override Activated. Resuming simulation.", "WARN")
            # Update the UI headers to show it's driving through the failure
    ...
    def update_simulation(self):
        ...
        if self.manual_mode:
            try:
                i, v, t = float(self.i_input[1].text()), float(self.v_input[1].text()), float(self.t_input[1].text())
            except:
                i, v, t = 0.0, 0.0, 0.0
            row = pd.Series(
                {'Current_RMS': i, 'VDC': v, 'Temp_Max': t, 'Temp_Diff_Max': 0.0, 'Ia': i, 'Power_DC': abs(v * i),
                 'T2': t, 'T3': t})
        else:
            if self.df.empty: return

            # Continue looping logic if enabled, otherwise stop
            ...
            i, v, t = row['Current_RMS'], row['VDC'], row['Temp_Max']
            self.data_index += 1

        z = abs(v / i) if i != 0 else 999.9;
        self.upd_plot("VDC", v);
        self.upd_plot("AMPS", i);
        self.upd_plot("TEMP", t);
        self.upd_plot("DELTA", row.get('Temp_Diff_Max', 0))

        l2_pred, l2_conf = self.compute_l2(row, z)
    ...
    def arbitrate(self, row, rules, i, v, t, z, l2_pred, l2_conf):
        ...
        if "CRITICAL" in verdict:
            # If continuous mode is NOT checked, do the normal Emergency Stop
            if not self.continuous_mode:
                self.paused = True

        self.update_user_dashboard(verdict)
        self.hidden_console.append(f"[{datetime.datetime.now().strftime('%H:%M:%S')}] Verdict: {verdict} | Z:{z:.2f}")

    def update_user_dashboard(self, v):
        if not hasattr(self, 'last_ui_state'): self.last_ui_state = ""

        # Don't repeat the same state updates repeatedly
        ...
        if "NOMINAL" in v:
            ...
            # UI Changes depending on if we bypassed the stop or not
            if self.continuous_mode:
                self.status_header.setText("CRITICAL FAULT (BYPASSED)")
                self.status_header.setStyleSheet("background-color: #7F1D1D; color: #FCA5A5")
            ...
            # 👉 Trigger Background Email regardless of bypass
    ...
    def export_report(self):
        fp, _ = QFileDialog.getSaveFileName(self, "Export Log", "XAI_Report.pdf", "PDF (*.pdf)")
        if not fp: return
        printer = QPrinter(QPrinter.ScreenResolution);
        printer.setOutputFormat(QPrinter.PdfFormat);
        printer.setOutputFileName(fp)
        self.hidden_console.document().print_(printer)
        self.push_notification(f"Forensic Log PDF Saved to {fp}", "SUCCESS")
